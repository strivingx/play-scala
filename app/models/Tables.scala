...
trait Tables {
  ...
  case class TUserRow(id: Long, userId: String, username: String, password: String, mail: Option[String] = None, gender: Option[Boolean] = None, age: Option[Int] = None)
  ...
  implicit def GetResultTUserRow(implicit e0: GR[Long], e1: GR[String], e2: GR[Option[String]], e3: GR[Option[Boolean]], e4: GR[Option[Int]]): GR[TUserRow] = GR{
    prs => import prs._
    TUserRow.tupled((<<[Long], <<[String], <<[String], <<[String], <<?[String], <<?[Boolean], <<?[Int]))
  }
  ...
  class TUser(_tableTag: Tag) extends profile.api.Table[TUserRow](_tableTag, Some("test"), "t_user") {
    def * = (id, userId, username, password, mail, gender, age) <> (TUserRow.tupled, TUserRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = (Rep.Some(id), Rep.Some(userId), Rep.Some(username), Rep.Some(password), mail, gender, age).shaped.<>({r=>import r._; _1.map(_=> TUserRow.tupled((_1.get, _2.get, _3.get, _4.get, _5, _6, _7)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
    ...
    /** Uniqueness Index over (userId) (database name un_user_id) */
    val index1 = index("un_user_id", userId, unique=true)
  }

...
*/
object ZookeeperUtil extends App {

    def getZookeeper(host: String = "localhost", port: Int = 2181, timeout:Int = 1000, watcher:Watcher = null): ZooKeeper = {
        // 创建一个与服务器的连接
        new ZooKeeper(s"$host:$port", 1000, new Watcher() {
            // 监控所有被触发的事件

...
/**
  ...
  */
class SnowflakeIdWorker(/** 工作机器ID(0~31) */
                        var workerId: Long,

                        /** 数据中心ID(0~31) */
                        var datacenterId: Long) {
    if (workerId > maxWorkerId || workerId < 0)
        throw new IllegalArgumentException(s"worker Id can't be greater than $maxWorkerId or less than 0")
    if (datacenterId > maxDatacenterId || datacenterId < 0)
        throw new IllegalArgumentException(s"datacenter Id can't be greater than $maxDatacenterId or less than 0")
    /** 开始时间截 (2015-01-01) */
    final private val twepoch = 1420041600000L
    /** 机器id所占的位数 */
    final private val workerIdBits = 5L
    /** 数据标识id所占的位数 */
    final private val datacenterIdBits = 5L
    /** 支持的最大机器id，结果是31 (这个移位算法可以很快的计算出几位二进制数所能表示的最大十进制数) */
    final private val maxWorkerId = -1L ^ (-1L << workerIdBits)
    /** 支持的最大数据标识id，结果是31 */
    final private val maxDatacenterId = -1L ^ (-1L << datacenterIdBits)
    /** 序列在id中占的位数 */
    final private val sequenceBits = 12L
    /** 机器ID向左移12位 */
    final private val workerIdShift = sequenceBits
    /** 数据标识id向左移17位(12+5) */
    final private val datacenterIdShift = sequenceBits + workerIdBits
    /** 时间截向左移22位(5+5+12) */
